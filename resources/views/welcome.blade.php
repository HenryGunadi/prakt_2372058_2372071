        ...
        <div class="space-y-6">
            ...
            @auth
                @if (auth('mahasiswa')->check())
                    <p class="text-lg font-medium">Logged in as: {{ auth('mahasiswa')->user()->nama }}</p>
                @elseif(auth('karyawan')->check())
                    <p class="text-lg font-medium">Logged in as: {{ auth('karyawan')->user()->nama }}</p>
                @else
                    <p class="text-lg font-medium">Logged in, but unknown role</p>
                @endif

                <a href="{{ url('/dashboard') }}"
                    class="inline-block px-6 py-2 bg-[#1b1b18] text-white dark:bg-white dark:text-black rounded-md hover:bg-[#333] dark:hover:bg-gray-200 transition">
                    Go to Dashboard
                </a>
            @else
